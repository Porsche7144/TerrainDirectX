...
void D3dClass::OnDepthBuffer()
{
	m_pDeviceContext->OMSetDepthStencilState(m_DepthStencilState.Get(), 1);
	return;
}

void D3dClass::OffDepthBuffer()
{
	m_pDeviceContext->OMSetDepthStencilState(m_DepthDisabledStencilState.Get(), 1);
	return;
}
...
Matrix D3dClass::GetProjectionMatrix(Matrix ProjMatrix)
{
	ProjMatrix = m_matProjection;
	return ProjMatrix;
}

Matrix D3dClass::GetWorldMatrix(Matrix matWorld)
{
	matWorld = m_matWorld;
	return matWorld;
}

Matrix D3dClass::GetOrthoMatrix(Matrix OrthoMatrix)
{
	...
	bool fullscreen, float screenDepth, float screenNear)
{
	...
	// 2D 렌더링을 위해 Z버퍼를 Off 하는 두번째 StencilState
	// 유일한 차이점은 DepthEnable을 false로 설정
	...
	// 설정한 깊이 스탠실 생성
	hr = m_pd3dDevice->CreateDepthStencilState(&DepthDisabledStencilDesc, m_DepthDisabledStencilState.GetAddressOf());
	if (FAILED(hr))
	{
		return false;
	}
